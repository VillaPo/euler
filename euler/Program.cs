    ...
    class Program
    {
        ...
        static byte[] OptimizedPrimeCount(int N)
        {
            ...
            for (int i = 2; i <= N; i++)
            {
                ...
                for (int p = 2; p <= i; p++)
                {
                    if (p * i > N) break;                               // если за пределами отрезка, бросаем это дело
                    if (lp[p] == 0)                                     // если p  простое   
                        lp[i * p] = (ushort)p;                          // все кратные ему числа будут иметь такой минимальный делитель (по идее, p не превысит
                                                                        // корень квадратный из N, т.е. 65535 должно хватить для отрезка до 4 294 836 225.
